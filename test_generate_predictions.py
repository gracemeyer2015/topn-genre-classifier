...
def make_fake_test_npz(tmp_path, n_samples=4):
    ...
    fake_X = np.random.randn(n_samples, 1, 128, 130).astype(np.float32)
...
def test_generate_predictions_returns_correct_length(tmp_path):
...
def test_generate_predictions_maps_true_labels_correctly(tmp_path):
